...
# Сколько пар «цель + блок цели» в настройках разметки (в st.form нельзя кнопку «+»).
_EXPORT_GOAL_UI_SLOTS = 10
...
def _parse_import_stop_block_id(item: dict, context_obj) -> int:
    """Конечный block_id для бенчмарка: корень кейса или context (`dm_stop_at_block_id` / `stop_at_block_id`)."""
    for src in (item, context_obj):
        if not isinstance(src, dict):
            continue
        for k in ("dm_stop_at_block_id", "stop_at_block_id"):
            v = src.get(k)
...
def _allowed_intents_str_from_goal_rows(rows: list) -> str:
    """Уникальные непустые тексты целей по порядку строк → строка для `allowed_intents`."""
    seen = set()
    out: list = []
    for r in rows or []:
...
def _merge_export_context(variables: dict, global_json: str, dialog_json: str) -> dict:
    """
    Итоговый context в JSONL: сначала переменные (снимок), затем общий context из настроек,
...
def _normalize_export_goal_rows(rows) -> list:
    """Ровно `_EXPORT_GOAL_UI_SLOTS` строк: справочник целей и block_id на цель (для экспорта по выбранной цели диалога)."""
    out = []
    if isinstance(rows, list):
        for r in rows:
            if len(out) >= _EXPORT_GOAL_UI_SLOTS:
                break
            if not isinstance(r, dict):
                continue
            out.append(
                {
                    ...
                    "stop_block_id": max(0, int(r.get("stop_block_id", 0) or 0)),
                }
            )
    while len(out) < _EXPORT_GOAL_UI_SLOTS:
        out.append({"text": "", "stop_block_id": 0})
    return out[:_EXPORT_GOAL_UI_SLOTS]


def _migrate_dialog_annotation(ann: dict, normalized_to_original: dict) -> None:
    """Убирает устаревшие поля; цель диалога только в intent_mode (из допустимого списка)."""
    if not isinstance(ann, dict):
        return
    gr = ann.pop("goal_rows", None)
    if gr and isinstance(gr, list) and gr and not (ann.get("intent_mode") or "").strip():
        first = gr[0]
        if isinstance(first, dict):
            ann["intent_mode"] = (first.get("text") or "").strip()
    ann.pop("bench_goals_text", None)
    ann.pop("bench_stop_block_id", None)
    raw = (ann.get("intent_mode") or "").strip()
    if raw and normalize_intent(raw) not in normalized_to_original:
        ann["intent_mode"] = ""


def _stop_block_id_for_export_goal(final_goal: str, rows: list) -> int:
    """
    `dm_stop_at_block_id` для кейса JSONL: берётся из строки настроек, у которой текст цели
    совпадает с размеченной целью диалога (сравнение через normalize_intent).
    """
    fg = (final_goal or "").strip()
    if not fg:
        return 0
    nk = normalize_intent(fg)
    for r in rows or []:
        if not isinstance(r, dict):
            continue
        ...
        if not rt:
            continue
        if normalize_intent(rt) != nk:
            continue
        try:
            b = int(r.get("stop_block_id", 0) or 0)
...
def _initial_goal_rows_from_item(item: dict, normalized_to_original: dict) -> list:
    """Стартовые строки целей из импорта: original_goals + import_dm_stop_at_block_id на первой строке."""
    ogs = item.get("original_goals")
    if ogs is None:
        ogs = []
    elif isinstance(ogs, str):
        ogs = [ogs] if str(ogs).strip() else []
    elif not isinstance(ogs, list):
        ogs = []
    imp_stop = max(0, int(item.get("import_dm_stop_at_block_id") or 0))
    rows = []
    for g in ogs:
        gs = str(g).strip() if g is not None else ""
        if not gs:
            continue
        nk = normalize_intent(gs)
        txt = normalized_to_original.get(nk, gs)
        rows.append({"text": txt, "stop_block_id": imp_stop if not rows else 0})
    if not rows:
        rows = [{"text": "", "stop_block_id": imp_stop}]
    return rows
...
def _parse_variable_name_list(text: str) -> list[str]:
    """Имена переменных из многострочного поля (строка или через запятую)."""
...
def _http_tool_tracked_variable_names() -> frozenset[str]:
    """Имена переменных из полей «Названия переменных (tool_name)» и «(tool_arguments)», объединённые."""
...
def _http_variables_allowlist_for_ui() -> frozenset[str] | None:
    """
    Если включён режим HTTP-блоков с перечислением имён — только эти переменные в снимке/UI/экспорте.
    Иначе None (показывать все переменные из лога).
    """
...
def _merge_timeline_message_id(a, b):
    if a is None:
        return b
    if b is None:
        return a
...
def _max_message_id_from_timeline(timeline: list) -> int | None:
    """Максимальный message_id среди реплик/тулов в срезе экспорта."""
    best = None
...
def snapshot_variables_latest_by_name(
    ...
    последняя запись по (message_id, created_at). Записи без variable_name пропускаются
    (в логе бывают только parameter_name).
    """
    rows = []
...
def parse_clickhouse_row(messages_str: str, tech_logs_str: str):
    """NDA: разбор колонок messages/tech_logs из ClickHouse скрыт."""
    return []
            ...
            for msg in timeline:
                if msg["type"] in ("tool_call", "tool_message", "tool_schema"):
                    ...
                    if (
                        ...
                        normalized_timeline[-1]["message_id"] = _merge_timeline_message_id(
                            normalized_timeline[-1].get("message_id"),
                            msg.get("message_id"),
                        )
                    else:
                        normalized_timeline.append(msg)
...
def build_block_chain_from_scenario(
    ...
    Раньше цепочка строилась по одному next_block_id на block_id (первое вхождение
    в массиве), из‑за петель вроде 33687→33687 получалось множество из одного блока.
    """
...
def _block_id_closest_to_anchor_time(by_mid: dict, msg_id, anchor_created_at: str):
    ...
    «Последний по времени» шаг для human часто относится к концу длинного HTTP/веток
    и визуально не соответствует тексту реплики; при равной близости берём более ранний шаг.
    """
    pairs = by_mid.get(msg_id, [])
    if not pairs:
        return None
...
def _bot_block_nearest_in_chain(
    ...
    Иначе text_blocks вне цепочки (например 33637) дают block_id, и реплики
    отсекаются фильтром, хотя идут внутри LLM-блока 33687.
    """
...
def _append_http_blocks_tool_calls_from_variables(
    ...
    """
    Шаги scenario_steps с block_type=http_blocks: если в variables есть записи с теми же
    message_id и subscenario_start_block_id, что и у шага, и значения отслеживаемых
    variable_name меняются относительно предыдущего снимка — добавляем synthetic tool_call.
    """
...
#             st.session_state["seen_dialog_ids"].add(item["dialog_id"])
    ...
    if source_option == "Загрузить готовый JSONL":
        ...
        _db_msg = st.session_state.pop("_markup05_db_loaded_msg", None)
        if _db_msg:
            st.success(_db_msg)

        def _on_markup05_text(text: str) -> None:
            st.session_state["_markup05_jsonl_text"] = text
            st.session_state["_markup05_db_loaded_msg"] = "Выборка загружена из БД."
            st.rerun()

        render_jsonl_dataset_source(
            key_prefix="markup05",
            on_text_loaded=_on_markup05_text,
            file_types=["jsonl"],
            upload_label="JSONL с диалогами",
        )
        jsonl_text = st.session_state.get("_markup05_jsonl_text")
        if not jsonl_text:
    ...
    try:
        ...
        for line_num, line in enumerate(jsonl_text.splitlines(), start=1):
            ...
            try:
                ...
                st.warning(f"⚠️ Некорректная строка {line_num}: {e}")
                continue
            ...
            for msg in history:
                ...
                tool_calls = extract_tool_calls_from_jsonl_history_message(msg)
                if tool_calls:
                    timeline.append(
                        {
                            "role": "assistant",
            ...
            vr = item.get("variables")
            ctx = item.get("context")
            extra_v = {}
            if isinstance(vr, list):
                extra_v["variables_raw"] = vr
            elif isinstance(vr, dict):
                extra_v["variables"] = vr
            elif isinstance(ctx, dict):
                extra_v["variables"] = ctx
            imp_stop = _parse_import_stop_block_id(item, ctx)
            dialog_groups.append(
                {
                    "dialog_id": dialog_id,
                    "scenario_id": scenario_id,
                    ...
                    "import_dm_stop_at_block_id": imp_stop,
                    **extra_v,
                }
            )
            # 🔥 Обновляем кэш просмотренных ID для JSONL
            if dialog_id:
                st.session_state["seen_dialog_ids"].add(dialog_id)
        records = []
        for item in dialog_groups:
            for msg in item["timeline"]:
                if msg["type"] == "message" and msg["content"].strip():
                    records.append(
                        {
                            "dialog_id": item["dialog_id"],
                            "role": msg["role"],
...
if "app_config" not in st.session_state:
    _export_seed = (
        _initial_goal_rows_from_item(dialog_groups[0], {})
        if dialog_groups
        else [{"text": "", "stop_block_id": 0}]
    )
    _export_seed = _normalize_export_goal_rows(_export_seed)
    st.session_state["app_config"] = {
        ...
        "allowed_intents": _allowed_intents_str_from_goal_rows(_export_seed),
        "export_goal_rows": _export_seed,
        "export_context_json": "{}",
    }

_cfg = st.session_state["app_config"]
if "export_goal_rows" not in _cfg:
    _a = _cfg.get("allowed_intents", "")
    _nti_m = {
        normalize_intent(g.strip()): g.strip()
        for g in str(_a).split(",")
        if g.strip()
    }
    _cfg["export_goal_rows"] = (
        _initial_goal_rows_from_item(dialog_groups[0], _nti_m)
        if dialog_groups
        else [{"text": "", "stop_block_id": 0}]
    )
_cfg["export_goal_rows"] = _normalize_export_goal_rows(_cfg.get("export_goal_rows"))
_cfg["allowed_intents"] = _allowed_intents_str_from_goal_rows(_cfg["export_goal_rows"])
...
with st.form("annotation_settings_form"):
    ...
    _erows = _normalize_export_goal_rows(config.get("export_goal_rows"))
    _ex_sig = tuple((r["text"], r["stop_block_id"]) for r in _erows)
    _slots_missing = any(
        f"form_cfg_grow_{_i}_text" not in st.session_state
        for _i in range(_EXPORT_GOAL_UI_SLOTS)
    )
    if st.session_state.get("_export_cfg_row_sig") != _ex_sig or _slots_missing:
        for _i, _r in enumerate(_erows):
            st.session_state[f"form_cfg_grow_{_i}_text"] = _r["text"]
            st.session_state[f"form_cfg_grow_{_i}_stop"] = int(_r["stop_block_id"])
        st.session_state["_export_cfg_row_sig"] = _ex_sig

    _hc0, _hc1 = st.columns([4, 1])
    with _hc0:
        st.caption("Цель ({goals})")
    with _hc1:
        st.caption("Блок цели")
    for _i in range(_EXPORT_GOAL_UI_SLOTS):
        _kt = f"form_cfg_grow_{_i}_text"
        _ks = f"form_cfg_grow_{_i}_stop"
        if _kt not in st.session_state:
            st.session_state[_kt] = _erows[_i]["text"] if _i < len(_erows) else ""
        if _ks not in st.session_state:
            st.session_state[_ks] = (
                int(_erows[_i]["stop_block_id"]) if _i < len(_erows) else 0
            )
        ...
        with _cc2:
            st.number_input(
                ...
                key=_ks,
                label_visibility="collapsed",
                help=">0 — в JSONL попадёт в dm_stop_at_block_id для диалогов с этой целью (совпадение по тексту цели).",
            )
    ...
    with st.expander("Пример задания context (JSON)", expanded=False):
        st.caption(
            ...
            "При экспорте эти поля добавляются к переменным диалога; совпадающие ключи может перезаписать context из карточки диалога."
        )
    ...
    st.text_area(
        ...
        placeholder='{\n  "branch": "центр",\n  "city": "Москва"\n}',
        help="Объект {...}. При экспорте объединяется с переменными диалога; совпадающие ключи перезаписываются context из карточки диалога.",
    )
    ...
    if submitted:
        ...
        if _gerr:
            ...
            _new_ex = []
            for _i in range(_EXPORT_GOAL_UI_SLOTS):
                _tx = (st.session_state.get(f"form_cfg_grow_{_i}_text", "") or "").strip()
                try:
                    _sb = int(st.session_state.get(f"form_cfg_grow_{_i}_stop", 0) or 0)
                except (TypeError, ValueError):
                    _sb = 0
                _new_ex.append({"text": _tx, "stop_block_id": max(0, _sb)})
            st.session_state["app_config"]["export_goal_rows"] = _new_ex
            st.session_state["app_config"]["allowed_intents"] = _allowed_intents_str_from_goal_rows(
                _new_ex
            )
            st.session_state["_export_cfg_row_sig"] = tuple(
                (r["text"], r["stop_block_id"]) for r in _new_ex
            )
...
for _did, _ann in st.session_state["annotations"].items():
    if _did not in current_ids:
        continue
    _migrate_dialog_annotation(_ann, normalized_to_original)
            ...
            with st.spinner("🔄 Выполнение разметки диалога..."):
                ...
                for item in selected_dialogs:
                    ...
                    if f_llm_mode == "Все диалоги":
                        ...
                        cg = (ann.get("intent_mode") or "").strip()
                        if not cg:
                            total_to_process += 1
                ...
                for idx, item in enumerate(selected_dialogs):
                    ...
                    current_goal_llm = (ann.get("intent_mode") or "").strip()
                    should_process = False
                    if f_llm_mode == "Все диалоги":
                        should_process = True
                    else:
                        if not current_goal_llm:
                            should_process = True
                    ...
                    if should_process:
                        ...
                        try:
                            ...
                            if norm_suggested in normalized_to_original:
                                ...
                                st.session_state["annotations"][dialog_id]["intent_mode"] = final_goal
...
for item in page_dialogs:
    ...
    if temp_keys["goal"] not in st.session_state["dialog_form_temp"]:
        st.session_state["dialog_form_temp"][temp_keys["goal"]] = (
            st.session_state["annotations"][dialog_id].get("intent_mode", "") or ""
        ).strip()
...
with st.form("dialog_annotation_form"):
    with st.expander("Пример дополнительного context в карточке диалога (JSON)", expanded=False):
        st.caption(
            ...
            "иначе объект объединяется поверх переменных и общего context."
        )
    ...
    for item in page_dialogs:
        ...
        cur_txt = (ann.get("intent_mode") or "").strip()
        in_allow = bool(cur_txt) and normalize_intent(cur_txt) in normalized_to_original
        if not cur_txt:
            goal_display = ""
            status_label = " ⚠️ [НЕРАЗМЕЧЕН]"
        elif not in_allow:
            goal_display = f" → {cur_txt}"
            status_label = " ⚠️ [ЦЕЛЬ ВНЕ СПИСКА]"
        else:
            goal_display = f" → {normalized_to_original[normalize_intent(cur_txt)]}"
            status_label = ""
        ...
        current_goal = (ann.get("intent_mode") or "").strip()
        ...
        with st.expander(
            ...
            st.selectbox(
                "🎯 Цель (разметка диалога)",
                options=options,
                index=options.index(current_goal) if current_goal in options else 0,
                key=key_goal,
                help="В JSONL в goals попадает только выбранная цель; dm_stop_at_block_id — из строки с той же целью в «Настройки разметки».",
            )
            ...
            st.text_area(
                ...
                key=key_dialog_ctx,
                placeholder='{\n  "caller_id": "+79001234567"\n}',
                help="Объект {...}. При экспорте: переменные диалога → общий context из настроек → этот JSON (ключи перезаписываются справа налево). См. пример в начале формы.",
            )
...
if submitted:
    ...
    if _ctx_bad:
        ...
        for item in page_dialogs:
            ...
            ag = applied_goal
            if ag:
                st.session_state[f"goal_{dialog_id}"] = ag
            else:
                st.session_state.pop(f"goal_{dialog_id}", None)
...
with col_b:
    ...
    for ann in st.session_state.get("annotations", {}).values():
        g = (ann.get("intent_mode") or "").strip()
...
export_data = []
normalized_to_original = {normalize_intent(g): g for g in allowed_intents}
_global_export_rows = _normalize_export_goal_rows(
    st.session_state["app_config"].get("export_goal_rows")
)
...
for item in selected_dialogs:
    ...
    # В JSONL — одна цель на диалог (разметка); block_id из строки настроек с совпадающим текстом.
    export_goals: list = [final_goal] if final_goal != "" else [""]
    ...
    _stop_bid = _stop_block_id_for_export_goal(final_goal, _global_export_rows)
    if _stop_bid > 0:
        export_item["dm_stop_at_block_id"] = _stop_bid
...
if export_data:
    ...
    render_export_jsonl_actions(
        jsonl_content,
        key_prefix="markup05_export",
        case_count=len(export_data),
        file_name="labeled_dialogs.jsonl",
        description="Размеченные диалоги (страница «Разметка из сценария»)",
        name_placeholder="labeled-scenario-v1",
    )
else:
